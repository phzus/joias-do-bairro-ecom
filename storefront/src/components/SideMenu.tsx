            ...
            {isAuthenticated && customer ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="text-2xl font-black uppercase tracking-tighter text-white group-hover:text-[#c8102e] transition-colors">
                ...
                <div className="flex gap-3">
                  <Link
                    to="/conta"
                    onClick={closeAndNavigate}
                    className="flex-1 flex items-center justify-center gap-2 py-3 text-[9px] font-bold uppercase tracking-widest bg-white text-black hover:bg-[#c8102e] hover:text-white transition-all"
                  >
                    <User size={12} />
                    Minha conta
                  </Link>
              ...
              <div className="space-y-4">
                ...
                <Link
                  to="/conta"
                  onClick={closeAndNavigate}
                  className="w-full inline-flex items-center justify-center gap-2 py-4 bg-[#c8102e] text-white text-[9px] font-bold uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all"
                >
                  <User size={12} />
                  Entrar / Cadastrar
                </Link>
            ...
            <div className="space-y-6">
              {categories.length === 0 && (
                <Link
                  to="/"
                  onClick={closeAndNavigate}
                  className="text-2xl font-black uppercase tracking-tighter text-white hover:text-[#c8102e] transition-colors"
                >
                  Todos os produtos
                </Link>
              )}
                  ...
                  <div key={cat.id} className="space-y-4">
                    <div className="flex items-center justify-between gap-4">
                      <Link
                        ...
                        onClick={closeAndNavigate}
                        className="text-2xl font-black uppercase tracking-tighter text-white hover:text-[#c8102e] transition-colors flex-1"
                      >
                        {cat.name}
                      </Link>
                      {hasChildren && (
                    ...
                    <AnimatePresence>
                      {isOpen && hasChildren && (
                        ...
                        >
                          <div className="flex flex-col gap-3 pl-4 border-l-2 border-[#c8102e]/30">
                            {cat.category_children!.map((sub) => (
                              ...
                              >
                                {sub.name}
                                <ArrowRight size={12} className="text-[#c8102e]/40" />
                              </Link>
                            ))}
                          </div>
                        </motion.div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <span className="text-[#c8102e]">{totalItems}</span>
              </Link>
              <Link
                to="/rastrear"
                onClick={closeAndNavigate}
