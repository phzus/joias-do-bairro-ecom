        ...
        <div className="container mx-auto px-6">
          <ul className="flex items-center justify-center space-x-16 h-14">
            {categories.map((cat, catIdx) => (
              ...
              >
                <button
                  ...
                  tabIndex={0}
                >
                  <span className={`w-1 h-1 bg-[#c8102e] rounded-full transition-opacity duration-300 ${activeMenu === cat.id ? 'opacity-100' : 'opacity-0'}`} />
                  {cat.name}
                  {cat.category_children && cat.category_children.length > 0 && (
                    <ChevronDown size={10} className={`transition-transform duration-500 ${activeMenu === cat.id ? 'rotate-180 text-[#c8102e]' : 'text-zinc-800'}`} />
                  )}
                </button>
                    ...
                    >
                      <div className="bg-zinc-950 border border-white/10 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.9)] p-10 min-w-[320px] relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                          <h4 className="text-[8px] font-black uppercase tracking-[0.6em] text-[#c8102e] mb-8 pb-3 border-b border-white/5 flex items-center justify-between">
                            ...
                            {cat.category_children.map((sub, idx) => (
                              ...
                              >
                                <Link
                                  ...
                                  aria-label={sub.name}
                                >
                                  <ArrowRight size={12} className="mr-3 text-[#c8102e] opacity-0 -ml-4 group-hover/item:opacity-100 group-hover/item:ml-0 transition-all" />
                                  {sub.name}
                                </Link>
                              </motion.div>
                            ))}
                          ...
                          <div className="mt-10 pt-6 border-t border-white/5">
                            <Link
                              to="/"
                              className="block w-full text-center py-4 bg-white text-black text-[9px] font-black uppercase tracking-[0.4em] hover:bg-[#c8102e] hover:text-white transition-all shadow-xl"
                              tabIndex={0}
            ...
            <li className="h-full flex items-center">
              <Link to="/" className="text-[10px] font-black uppercase tracking-[0.4em] text-[#c8102e] hover:scale-110 transition-transform flex items-center gap-2" tabIndex={0} aria-label="Drops limitados">
                <span className="w-2 h-2 bg-[#c8102e] animate-pulse rounded-full" />
                Drop Limitado
              </Link>
            </li>
          </ul>
        ...
        <div className={`absolute top-0 left-0 h-full w-[85%] max-w-sm bg-black border-r border-white/5 transform transition-transform duration-500 p-10 pt-24 overflow-y-auto ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex flex-col space-y-12">
            {categories.map((cat) => (
              <div key={cat.id} className="space-y-6">
                <button
                  ...
                  tabIndex={0}
                >
                  <span className={`text-2xl font-black italic uppercase tracking-tighter transition-colors ${activeMenu === cat.id ? 'text-[#c8102e]' : 'text-white'}`}>
                    {cat.name}
                  </span>
                  {cat.category_children && cat.category_children.length > 0 && (
                    <ChevronDown size={20} className={`text-zinc-800 transition-transform duration-500 ${activeMenu === cat.id ? 'rotate-180 text-[#c8102e]' : ''}`} />
                  )}
                </button>
                ...
                <AnimatePresence>
                  {activeMenu === cat.id && cat.category_children && cat.category_children.length > 0 && (
                    ...
                    >
                      <div className="flex flex-col space-y-5 pl-4 border-l-2 border-[#c8102e]/30 py-2">
                        {cat.category_children.map((sub) => (
                          ...
                          >
                            {sub.name}
                            <ArrowRight size={14} className="text-[#c8102e]/30" />
                          </Link>
                        ))}
                      </div>
                    </motion.div>
            ...
            <Link
              to="/"
              onClick={onClose}
              className="text-2xl font-black italic uppercase tracking-tighter text-[#c8102e] animate-pulse"
              tabIndex={0}
