  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-10 left-10 z-[110] bg-black border border-white/20 p-4 hover:border-[#c8102e] group transition-all"
      >
        <div className="flex items-center gap-3">
          <Cpu size={20} className="text-[#c8102e] group-hover:animate-pulse" />
              ...
              <div className="border border-white/5 p-8 relative z-10 bg-zinc-950">
                <div className="flex justify-between items-start mb-12">
                  <div className="flex items-center gap-4">
                    <Terminal size={24} className="text-[#c8102e]" />
                    <div>
                ...
                <div className="bg-black/50 p-6 mb-8 font-mono text-[10px] space-y-2 h-40 overflow-y-auto border border-white/5 scrollbar-hide">
                  {history.map((line, i) => (
                    <div key={i} className={line.startsWith('>') ? 'text-[#c8102e]' : 'text-zinc-500'}>{line}</div>
                  ))}
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-transparent border-b-2 border-white/10 py-4 font-mono text-sm focus:outline-none focus:border-[#c8102e] transition-all uppercase placeholder:text-zinc-800"
                    disabled={isLoading}
                  />
                  <button
                    onClick={processVibe}
                    disabled={isLoading}
                    className="absolute right-0 bottom-4 text-[#c8102e] hover:text-white transition-colors"
                  >
                    <Zap size={20} />
                  </button>
                </div>
